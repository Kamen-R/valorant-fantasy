...
export default function SwapButton({ lid, tid }) {
  ...
  const updatePos = async () => {
    ...
    var { error } = await supabase.from('Teams').update({[pos1.id]: pos2.innerHTML, [pos2.id]: pos1.innerHTML}).eq('lid', lid).eq('tid', tid)
    ...
    for (const matchup of data) {
      ...
      var { error } = await supabase.from('Matchups').update({[team_num+pos1.id]: pos2.innerHTML, [team_num+pos2.id]: pos1.innerHTML}).eq('mid', mid)
    }
